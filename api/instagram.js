...
// Single request per call; aggressively cached to avoid rate limits.
...
export default async function handler(req, res) {
    ...
    // Cache aggressively — 1 hour on Vercel edge, serve stale for another hour
    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate=3600');

    try {
        // Single request — GET requests don't require csrftoken
        ...
        if (apiRes.status === 429) {
            // Return stale cache if available, otherwise 429
            res.setHeader('Cache-Control', 'stale-if-error=86400');
            return res.status(429).json({ error: 'Instagram rate limit hit — try again in a few minutes', rateLimit: true });
        }
        ...
        if (!apiRes.ok) {
            ...
            console.error(`IG ${apiRes.status} for ${username}:`, body.slice(0, 200));
